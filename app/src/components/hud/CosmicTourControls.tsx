      ...
      {!isTourActive ? (
        <>
          <button
            ...
            title={audioError ?? (isAudioReady ? 'Sinematik Uzay Turunu Başlat' : 'Sinematik ses yükleniyor')}
          >
            ...
            <span className="hidden md:inline tracking-wider uppercase">
              {isAudioReady ? '🚀 SİNEMATİK UZAY TURUNU BAŞLAT' : '⏳ SİNEMATİK SES YÜKLENİYOR'}
            </span>
            <span className="md:hidden text-sm">🚀</span>
          </button>
          <div className="flex overflow-hidden rounded-md border border-cyan-400/25 font-mono text-[10px]" role="group" aria-label="Sinematik tur dili">
            {(['tr', 'en'] as const).map((option) => (
              <button
                ...
                title={option === 'tr' ? 'Türkçe seslendirme' : 'English narration'}
              >
                {option === 'tr' ? 'TR' : 'ENG'}
              </button>
            ))}
